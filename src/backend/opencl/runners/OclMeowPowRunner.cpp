...
#include "backend/opencl/kernels/meowpow/MeowPow_CalculateDAGKernel.h"
...
OclMeowPowRunner::OclMeowPowRunner(size_t index, const OclLaunchData &data) : OclBaseRunner(index, data)
{
...
OclMeowPowRunner::~OclMeowPowRunner()
{
...
void OclMeowPowRunner::run(uint32_t nonce, uint32_t *hashOutput)
{
...
void OclMeowPowRunner::set(const Job &job, uint8_t *blob)
{
    m_blockHeight = static_cast<uint32_t>(job.height());
    m_searchKernel = OclMeowPow::get(*this, m_blockHeight, m_workGroupSize);

    const uint32_t epoch = m_blockHeight / MPHash::EPOCH_LENGTH;

    const uint64_t dag_size = MPCache::dag_size(epoch);
    ...
    if (epoch != m_epoch) {
        ...
        {
            std::lock_guard<std::mutex> lock(MPCache::s_cacheMutex);

            MPCache::s_cache.init(epoch);

            if (MPCache::s_cache.size() > m_lightCacheCapacity) {
                OclLib::release(m_lightCache);

                m_lightCacheCapacity = VirtualMemory::align(MPCache::s_cache.size());
                m_lightCache = OclLib::createBuffer(m_ctx, CL_MEM_READ_ONLY, m_lightCacheCapacity);
            }

            m_lightCacheSize = MPCache::s_cache.size();
            enqueueWriteBuffer(m_lightCache, CL_TRUE, 0, m_lightCacheSize, MPCache::s_cache.data());
        }
        ...
        double dag_in_gb = (double)MPCache::dag_size(epoch) / (1024.0 * 1024.0 * 1024.0);
        LOG_INFO("%s " YELLOW("MeowPow") " DAG for epoch " WHITE_BOLD("%u") " calculated " BLACK_BOLD("(%" PRIu64 "ms)") " %.2f GB", Tags::opencl(), epoch, Chrono::steadyMSecs() - start_ms, dag_in_gb);
    }
...
void OclMeowPowRunner::jobEarlyNotification(const Job&)
{
...
void xmrig::OclMeowPowRunner::build()
{
    OclBaseRunner::build();

    m_calculateDagKernel = new MeowPow_CalculateDAGKernel(m_program);
}


void xmrig::OclMeowPowRunner::init()
{
    OclBaseRunner::init();
