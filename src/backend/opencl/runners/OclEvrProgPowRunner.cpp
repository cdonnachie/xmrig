...
#include "backend/opencl/kernels/evrprogpow/EvrProgPow_CalculateDAGKernel.h"
...
OclEvrProgPowRunner::OclEvrProgPowRunner(size_t index, const OclLaunchData &data) : OclBaseRunner(index, data)
{
...
OclEvrProgPowRunner::~OclEvrProgPowRunner()
{
...
void OclEvrProgPowRunner::run(uint32_t nonce, uint32_t *hashOutput)
{
...
void OclEvrProgPowRunner::set(const Job &job, uint8_t *blob)
{
    m_blockHeight = static_cast<uint32_t>(job.height());
    m_searchKernel = OclEvrProgPow::get(*this, m_blockHeight, m_workGroupSize);

    const uint32_t epoch = m_blockHeight / EPPHash::EPOCH_LENGTH;

    const uint64_t dag_size = EPPCache::dag_size(epoch);
    ...
    if (epoch != m_epoch) {
        ...
        {
            std::lock_guard<std::mutex> lock(EPPCache::s_cacheMutex);

            EPPCache::s_cache.init(epoch);

            if (EPPCache::s_cache.size() > m_lightCacheCapacity) {
                OclLib::release(m_lightCache);

                m_lightCacheCapacity = VirtualMemory::align(EPPCache::s_cache.size());
                m_lightCache = OclLib::createBuffer(m_ctx, CL_MEM_READ_ONLY, m_lightCacheCapacity);
            }

            m_lightCacheSize = EPPCache::s_cache.size();
            enqueueWriteBuffer(m_lightCache, CL_TRUE, 0, m_lightCacheSize, EPPCache::s_cache.data());
        }
        ...
        LOG_INFO("%s " YELLOW("EvrProgPow") " DAG for epoch " WHITE_BOLD("%u") " calculated " BLACK_BOLD("(%" PRIu64 "ms)"), Tags::opencl(), epoch, Chrono::steadyMSecs() - start_ms);
    }
...
void OclEvrProgPowRunner::jobEarlyNotification(const Job&)
{
...
void xmrig::OclEvrProgPowRunner::build()
{
    OclBaseRunner::build();

    m_calculateDagKernel = new EvrProgPow_CalculateDAGKernel(m_program);
}


void xmrig::OclEvrProgPowRunner::init()
{
    OclBaseRunner::init();
