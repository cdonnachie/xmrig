...
#include "backend/opencl/runners/tools/OclEvrProgPow.h"
#include "3rdparty/libethash/data_sizes.h"
#include "3rdparty/libethash/ethash_internal.h"
#include "backend/opencl/cl/evrprogpow/evrprogpow_cl.h"
...
class EvrProgPowCacheEntry
{
public:
    inline EvrProgPowCacheEntry(const Algorithm &algo, uint64_t period, uint32_t worksize, uint32_t index, cl_program program, cl_kernel kernel) :
        program(program),
        kernel(kernel),
        m_algo(algo),
        m_index(index),
...
class EvrProgPowCache
{
public:
    EvrProgPowCache() = default;
...
private:
    ...
    std::mutex m_mutex;
    std::vector<EvrProgPowCacheEntry> m_data;
};


static EvrProgPowCache cache;


#define rnd()       (kiss99(rnd_state))
#define mix_src()   ("mix[" + std::to_string(rnd() % EPPHash::REGS) + "]")
#define mix_dst()   ("mix[" + std::to_string(mix_seq_dst[(mix_seq_dst_cnt++) % EPPHash::REGS]) + "]")
#define mix_cache() ("mix[" + std::to_string(mix_seq_cache[(mix_seq_cache_cnt++) % EPPHash::REGS]) + "]")

class EvrProgPowBaton : public Baton<uv_work_t>
{
public:
    inline EvrProgPowBaton(const IOclRunner& runner, uint64_t period, uint32_t worksize) :
        runner(runner),
        period(period),
        worksize(worksize)
    {}
...
public:
    ...
    cl_kernel build(const IOclRunner &runner, uint64_t period, uint32_t worksize)
    {
        ...
        const uint64_t epoch = (period * EPPHash::PERIOD_LENGTH) / EPPHash::EPOCH_LENGTH;
        const uint64_t dag_elements = (dag_sizes_evr[epoch]) / 256;
        ...
        LOG_INFO("%s " YELLOW("EvrProgPow") " program for period " WHITE_BOLD("%" PRIu64) " compiled " BLACK_BOLD("(%" PRIu64 "ms)"), Tags::opencl(), period, Chrono::steadyMSecs() - ts);
...
private:
    ...
    static std::string getSource(uint64_t prog_seed)
    {
        ...
        int mix_seq_dst[EPPHash::REGS];
        int mix_seq_cache[EPPHash::REGS];
        int mix_seq_dst_cnt = 0;
        int mix_seq_cache_cnt = 0;

        for (uint32_t i = 0; i < EPPHash::REGS; i++) {
            mix_seq_dst[i] = i;
            mix_seq_cache[i] = i;
        }

        for (int i = EPPHash::REGS - 1; i > 0; i--) {
            int j = 0;
            j = rnd() % (i + 1);
        ...
        for (int i = 0; (i < EPPHash::CNT_CACHE) || (i < EPPHash::CNT_MATH); ++i) {
            if (i < EPPHash::CNT_CACHE) {
            ...
            if (i < EPPHash::CNT_MATH) {
                // Random Math
                // Generate 2 unique sources
                int src_rnd = rnd() % ((EPPHash::REGS - 1) * EPPHash::REGS);
                int src1 = src_rnd % EPPHash::REGS; // 0 <= src1 < EPPHash::REGS
                int src2 = src_rnd / EPPHash::REGS; // 0 <= src2 < EPPHash::REGS - 1
        ...
        std::string kernel = std::regex_replace(std::string(evrprogpow_cl), std::regex("XMRIG_INCLUDE_PROGPOW_RANDOM_MATH"), ret.str());
        ret.str(std::string());

        ret << merge("mix[0]", "data_dag.s[0]", rnd());

        constexpr size_t num_words_per_lane = 256 / (sizeof(uint32_t) * EPPHash::LANES);
...
private:
    ...
    std::vector<EvrProgPowBaton> m_batons;

    static void loop(void* data)
    {
        EvrProgPowBuilder* builder = static_cast<EvrProgPowBuilder*>(data);
...
static EvrProgPowBuilder builder;


void EvrProgPowBuilder::build_async(const IOclRunner& runner, uint64_t period, uint32_t worksize)
{
    std::lock_guard<std::mutex> lock(m_mutex);
    ...
    if (!m_loop) {
        ...
        uv_async_init(m_loop, &m_shutdownAsync, [](uv_async_t* handle)
            {
                EvrProgPowBuilder* builder = reinterpret_cast<EvrProgPowBuilder*>(handle->data);
        ...
        uv_async_init(m_loop, &m_batonAsync, [](uv_async_t* handle)
            {
                std::vector<EvrProgPowBaton> batons;
                {
                    EvrProgPowBuilder* b = reinterpret_cast<EvrProgPowBuilder*>(handle->data);

                    std::lock_guard<std::mutex> lock(b->m_mutex);
                    batons = std::move(b->m_batons);
                }

                for (const EvrProgPowBaton& baton : batons) {
...
cl_kernel OclEvrProgPow::get(const IOclRunner &runner, uint64_t height, uint32_t worksize)
{
    const uint64_t period = height / EPPHash::PERIOD_LENGTH;
