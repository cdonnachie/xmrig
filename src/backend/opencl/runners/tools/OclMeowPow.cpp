...
#include "backend/opencl/runners/tools/OclMeowPow.h"
...
class MeowPowCacheEntry
{
public:
    inline MeowPowCacheEntry(const Algorithm &algo, uint64_t period, uint32_t worksize, uint32_t index, cl_program program, cl_kernel kernel) :
        program(program),
        kernel(kernel),
        m_algo(algo),
        m_index(index),
...
private:
    ...
    std::mutex m_mutex;
    std::vector<MeowPowCacheEntry> m_data;
};


static MeowPowCache cache;


#define rnd()       (kiss99(rnd_state))
#define mix_src()   ("mix[" + std::to_string(rnd() % MPHash::REGS) + "]")
#define mix_dst()   ("mix[" + std::to_string(mix_seq_dst[(mix_seq_dst_cnt++) % MPHash::REGS]) + "]")
#define mix_cache() ("mix[" + std::to_string(mix_seq_cache[(mix_seq_cache_cnt++) % MPHash::REGS]) + "]")

class MeowPowBaton : public Baton<uv_work_t>
{
public:
    inline MeowPowBaton(const IOclRunner& runner, uint64_t period, uint32_t worksize) :
        runner(runner),
        period(period),
        worksize(worksize)
    {}
...
public:
    ...
    cl_kernel build(const IOclRunner &runner, uint64_t period, uint32_t worksize)
    {
        ...
        const uint64_t epoch = (period * MPHash::PERIOD_LENGTH) / MPHash::EPOCH_LENGTH;
        ...
        LOG_INFO("%s " YELLOW("MeowPow") " program for period " WHITE_BOLD("%" PRIu64) " compiled " BLACK_BOLD("(%" PRIu64 "ms)"), Tags::opencl(), period, Chrono::steadyMSecs() - ts);
...
private:
    ...
    static std::string getSource(uint64_t prog_seed)
    {
        ...
        int mix_seq_dst[MPHash::REGS];
        int mix_seq_cache[MPHash::REGS];
        int mix_seq_dst_cnt = 0;
        int mix_seq_cache_cnt = 0;

        for (uint32_t i = 0; i < MPHash::REGS; i++) {
            mix_seq_dst[i] = i;
            mix_seq_cache[i] = i;
        }

        for (int i = MPHash::REGS - 1; i > 0; i--) {
            int j = 0;
            j = rnd() % (i + 1);
        ...
        for (int i = 0; (i < MPHash::CNT_CACHE) || (i < MPHash::CNT_MATH); ++i) {
            if (i < MPHash::CNT_CACHE) {
            ...
            if (i < MPHash::CNT_MATH) {
                // Random Math
                // Generate 2 unique sources
                int src_rnd = rnd() % ((MPHash::REGS - 1) * MPHash::REGS);
                int src1 = src_rnd % MPHash::REGS; // 0 <= src1 < MPHash::REGS
                int src2 = src_rnd / MPHash::REGS; // 0 <= src2 < MPHash::REGS - 1
        ...
        std::string kernel = std::regex_replace(std::string(meowpow_cl), std::regex("XMRIG_INCLUDE_PROGPOW_RANDOM_MATH"), ret.str());
        ret.str(std::string());

        ret << merge("mix[0]", "data_dag.s[0]", rnd());

        constexpr size_t num_words_per_lane = 256 / (sizeof(uint32_t) * MPHash::LANES);
...
private:
    ...
    std::vector<MeowPowBaton> m_batons;

    static void loop(void* data)
    {
        MeowPowBuilder* builder = static_cast<MeowPowBuilder*>(data);
...
static MeowPowBuilder builder;


void MeowPowBuilder::build_async(const IOclRunner& runner, uint64_t period, uint32_t worksize)
{
    std::lock_guard<std::mutex> lock(m_mutex);
    ...
    if (!m_loop) {
        ...
        uv_async_init(m_loop, &m_shutdownAsync, [](uv_async_t* handle)
            {
                MeowPowBuilder* builder = reinterpret_cast<MeowPowBuilder*>(handle->data);
        ...
        uv_async_init(m_loop, &m_batonAsync, [](uv_async_t* handle)
            {
                std::vector<MeowPowBaton> batons;
                {
                    MeowPowBuilder* b = reinterpret_cast<MeowPowBuilder*>(handle->data);

                    std::lock_guard<std::mutex> lock(b->m_mutex);
                    batons = std::move(b->m_batons);
                }

                for (const MeowPowBaton& baton : batons) {
...
cl_kernel OclMeowPow::get(const IOclRunner &runner, uint64_t height, uint32_t worksize)
{
    const uint64_t period = height / MPHash::PERIOD_LENGTH;
