...
#include "backend/opencl/kernels/meraki/Meraki_CalculateDAGKernel.h"
...
OclMerakiRunner::OclMerakiRunner(size_t index, const OclLaunchData &data) : OclBaseRunner(index, data)
{
...
OclMerakiRunner::~OclMerakiRunner()
{
...
void OclMerakiRunner::run(uint32_t nonce, uint32_t *hashOutput)
{
...
void OclMerakiRunner::set(const Job &job, uint8_t *blob)
{
    m_blockHeight = static_cast<uint32_t>(job.height());
    m_searchKernel = OclMeraki::get(*this, m_blockHeight, m_workGroupSize);

    const uint32_t epoch = m_blockHeight / MKHash::EPOCH_LENGTH;

    const uint64_t dag_size = MKCache::dag_size(epoch);
    ...
    if (epoch != m_epoch) {
...

        {
            ...
            MKCache::s_cache.init(epoch);

            if (MKCache::s_cache.size() > m_lightCacheCapacity) {
                OclLib::release(m_lightCache);

                m_lightCacheCapacity = VirtualMemory::align(MKCache::s_cache.size());
                m_lightCache = OclLib::createBuffer(m_ctx, CL_MEM_READ_ONLY, m_lightCacheCapacity);
            }

            m_lightCacheSize = MKCache::s_cache.size();
            enqueueWriteBuffer(m_lightCache, CL_TRUE, 0, m_lightCacheSize, MKCache::s_cache.data());
        }
        ...
        LOG_INFO("%s " YELLOW("Meraki") " DAG for epoch " WHITE_BOLD("%u") " calculated " BLACK_BOLD("(%" PRIu64 "ms)"), Tags::opencl(), epoch, Chrono::steadyMSecs() - start_ms);
    }
...
void OclMerakiRunner::jobEarlyNotification(const Job&)
{
...
void xmrig::OclMerakiRunner::build()
{
    OclBaseRunner::build();

    m_calculateDagKernel = new Meraki_CalculateDAGKernel(m_program);
}


void xmrig::OclMerakiRunner::init()
{
    OclBaseRunner::init();
