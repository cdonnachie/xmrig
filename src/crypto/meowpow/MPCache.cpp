...
std::mutex MPCache::s_cacheMutex;
MPCache MPCache::s_cache;
...
bool MPCache::init(uint32_t epoch)
{
    ...
    if (meowpow_epoch >= sizeof(cache_sizes) / sizeof(cache_sizes[0])) {
        return false;
    }
    ...
    const size_t size = cache_sizes[meowpow_epoch];
    ...
    LOG_INFO("%s " YELLOW("MeowPow") " light cache for epoch " WHITE_BOLD("%u") " calculated " BLACK_BOLD("(%" PRIu64 "ms)"), Tags::miner(), epoch, Chrono::steadyMSecs() - start_ms);

    return true;
}


void* MPCache::data() const
{
...
uint64_t MPCache::cache_size(uint32_t epoch)
{
    uint32_t meowpow_epoch = epoch;
    if (epoch >= MPHash::DAGCHG_EPOCH) {
        meowpow_epoch = epoch * 4;
    ...
    return cache_sizes[meowpow_epoch];
}


uint64_t MPCache::dag_size(uint32_t epoch)
{
    uint32_t meowpow_epoch = epoch;
    if (epoch >= MPHash::DAGCHG_EPOCH) {
        meowpow_epoch = epoch * 4;
    }
    if (meowpow_epoch >= sizeof(dag_sizes) / sizeof(dag_sizes[0])) {
        return 0;
    }

    return dag_sizes[meowpow_epoch];
}


void MPCache::calculate_fast_mod_data(uint32_t divisor, uint32_t& reciprocal, uint32_t& increment, uint32_t& shift)
{
