...
#ifndef XMRIG_EPP_HASH_H
#define XMRIG_EPP_HASH_H
...
{
public:
    static constexpr uint32_t EPOCH_LENGTH  = 12000;
    ...
    static void calculate(const EPPCache& light_cache, uint32_t block_height, const uint8_t (&header_hash)[32], uint64_t nonce, uint32_t (&output)[8], uint32_t (&mix_hash)[8]);
};
