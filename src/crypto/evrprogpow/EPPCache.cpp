...
std::mutex EPPCache::s_cacheMutex;
EPPCache EPPCache::s_cache;
...
bool EPPCache::init(uint32_t epoch)
{
    ...
    LOG_INFO("%s " YELLOW("EvrProgPow") " light cache for epoch " WHITE_BOLD("%u") " calculated " BLACK_BOLD("(%" PRIu64 "ms)"), Tags::miner(), epoch, Chrono::steadyMSecs() - start_ms);

    return true;
}


void* EPPCache::data() const
{
...
uint64_t EPPCache::cache_size(uint32_t epoch)
{
    if (epoch >= sizeof(cache_sizes) / sizeof(cache_sizes[0])) {
        return 0;
    }
...
uint64_t EPPCache::dag_size(uint32_t epoch)
{
    if (epoch >= sizeof(dag_sizes_evr) / sizeof(dag_sizes_evr[0])) {
        return 0;
    }

    return dag_sizes_evr[epoch];
}


void EPPCache::calculate_fast_mod_data(uint32_t divisor, uint32_t& reciprocal, uint32_t& increment, uint32_t& shift)
{
