...
void EPPHash::calculate(const EPPCache& light_cache, uint32_t block_height, const uint8_t (&header_hash)[32], uint64_t nonce, uint32_t (&output)[8], uint32_t (&mix_hash)[8])
{
    ...
    memcpy(keccak_state + 10, evrmore_evrprogpow, sizeof(evrmore_evrprogpow));
    ...
    const uint32_t prog_number = block_height / EPPHash::PERIOD_LENGTH;
    ...
    const uint32_t num_items = static_cast<uint32_t>((dag_sizes_evr[epoch]) / ETHASH_MIX_BYTES / 2);
    ...
    EPPCache::calculate_fast_mod_data(cache.num_parent_nodes, cache.reciprocal, cache.increment, cache.shift);
    ...
    for (uint32_t r = 0; r < ETHASH_ACCESSES; ++r) {
        ...
        node item[4];
        ethash_calculate_dag_item4_opt(item, item_index, EPPCache::num_dataset_parents, &cache);
        ...
        for (uint32_t i = 0; i < max_operations; ++i) {
            if (i < CNT_CACHE) {
                ...
                for (uint32_t j = 0; j < LANES; ++j) {
                    random_merge(mix[j][dst], light_cache.l1_cache()[mix[j][src] % EPPCache::l1_cache_num_items], sel);
                }
            }
    ...
    memcpy(keccak_state + 16, evrmore_evrprogpow, sizeof(uint32_t) * 9);
