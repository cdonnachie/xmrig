...
static const uint32_t ravencoin_meraki[15] = {
        0x00000072, //R
        0x00000041, //A
        0x00000056, //V
        0x00000045, //E
...
void MKHash::calculate(const MKCache& light_cache, uint32_t block_height, const uint8_t (&header_hash)[32], uint64_t nonce, uint32_t (&output)[8], uint32_t (&mix_hash)[8])
{
    ...
    memcpy(keccak_state + 10, ravencoin_meraki, sizeof(ravencoin_meraki));
    ...
    const uint32_t prog_number = block_height / MKHash::PERIOD_LENGTH;
    ...
    MKCache::calculate_fast_mod_data(cache.num_parent_nodes, cache.reciprocal, cache.increment, cache.shift);
    ...
    for (uint32_t r = 0; r < ETHASH_ACCESSES_TLS; ++r) {
        uint32_t item_index = (mix[r % LANES][0] % num_items) * 4;

        node item[4];
        ethash_calculate_dag_item4_opt(item, item_index, MKCache::num_dataset_parents, &cache);
        ...
        for (uint32_t i = 0; i < max_operations; ++i) {
            if (i < CNT_CACHE) {
                ...
                for (uint32_t j = 0; j < LANES; ++j) {
                    random_merge(mix[j][dst], light_cache.l1_cache()[mix[j][src] % MKCache::l1_cache_num_items], sel);
                }
            }
    ...
    memcpy(keccak_state + 16, ravencoin_meraki, sizeof(uint32_t) * 9);
