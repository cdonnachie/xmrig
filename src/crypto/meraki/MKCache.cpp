...
std::mutex MKCache::s_cacheMutex;
MKCache MKCache::s_cache;
...
bool MKCache::init(uint32_t epoch)
{
    ...
    LOG_INFO("%s " YELLOW("Meraki") " light cache for epoch " WHITE_BOLD("%u") " calculated " BLACK_BOLD("(%" PRIu64 "ms)"), Tags::miner(), epoch, Chrono::steadyMSecs() - start_ms);

    return true;
}


void* MKCache::data() const
{
...
uint64_t MKCache::cache_size(uint32_t epoch)
{
    if (epoch >= sizeof(cache_sizes) / sizeof(cache_sizes[0])) {
        return 0;
    }
...
uint64_t MKCache::dag_size(uint32_t epoch)
{
...
void MKCache::calculate_fast_mod_data(uint32_t divisor, uint32_t& reciprocal, uint32_t& increment, uint32_t& shift)
{
