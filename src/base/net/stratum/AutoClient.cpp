...
bool xmrig::AutoClient::parseLogin(const rapidjson::Value &result, int *code)
{
    ...
    if (algo.family() != Algorithm::KAWPOW && algo.family() != Algorithm::MEOWPOW && algo.family() != Algorithm::EVRPROGPOW && algo.family() != Algorithm::MERAKI && algo.family() != Algorithm::GHOSTRIDER) {
        *code = 6;
        return false;
    }
